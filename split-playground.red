...
context [
	...
	view [
		...
		return
		pad 0x-10
		drop-down 620 data tests on-change [
		...
		return
		pad 0x-10
		input-series:    text 300 white ;input to split
		pad 10x0
		expected-result: text 300 white ;expected result
		return pad -10x0
		panel [ ;Gregg's
			below
			text 300x30 wrap "Please specify appropriate delimiter for dialected split:"
			dialected-delimiter: field 300 [  ;delim DSL
				local [result]
			...
			text "Result:"
			pad 0x-10
			dialected-result: text  300 white
		]
		pad -10x0
		panel [ ;Toomas'
			...
			refinement-delimiter: field 300 [
				local [result]
				refinement-result/text: mold result: either empty? refinements/data [
					split-r load input-series/text do face/data
				][
					split-r/with load input-series/text do face/data refinements/data
				]
				refinement-result/color: reduce pick [leaf brick] equal? result selected/goal
			]
			return
			refs: panel [
				origin 0x0 
			...
			return
			pad 0x-10
			refinement-result: text 300 white
		]
		return
		button "Quit" [unview]
	]
